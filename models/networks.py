...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='xavier', init_gain=0.02,  gpu_ids=[], nc=256):
    if netF == 'sample':
    ...
    elif netF == 'mlp_sample_cut':
        net = PatchSampleF_CUT(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=nc)
    else:
...
def get_local_index(x, patch_id, k_size=3):
    loc_weight = []
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=True, init_type='xavier', init_gain=0.02, nc=256, gpu_ids=[0]):
        ...
        self.k_size= [int(i) for i in '3'.split(',')]
        ...
        # self.mlp3 = nn.Sequential(*[nn.Linear(256, self.nc), nn.ReLU(), nn.Linear(self.nc, self.nc)])
        self.mlp3 = nn.Sequential(
                nn.Linear(256, mlp_hidden_size),
                nn.BatchNorm1d(mlp_hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(mlp_hidden_size, projection_size)
            )
    ...
    def forward(self, patch_size,feats, num_patches,patch_ids=None,patch_local_ids=None,top_idxs=None,pixweght=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if patch_ids is not None:
                ...
                local_id = get_local_index(feat_reshape,patch_id=patch_id, k_size=self.k_size[0]).to(feat_reshape.device)
            ...
            if top_idxs is not None:
                ...
                top_idx = sim_matrix.squeeze().topk(k=k_num[0],dim=-1)[1].view(-1,k_num[0],1)
            ...
            if (h==patch_size//16) or (h==patch_size//4):
            # if (h==patch_size//16):
                x_sample = F.adaptive_avg_pool2d(torch.cat((x_center,x_loc),dim=1),(1,dim))

            # if (h==patch_size) or (h==patch_size//4):
            if (h==patch_size):
...
class Frequency_Attention(nn.Module):
    def __init__(self, channel=2, kernel_size=5, bias=False):
        super(Frequency_Attention, self).__init__()

        self.spatial_conv = nn.Conv2d(channel, 1, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, bias=bias)
    ...
    def forward(self, x):
        ...
        spatial_weights = self.sigmoid(self.spatial_conv(channel_pool))
...
class Gradient_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        gradient_y = F.conv2d(x, weight=torch.Tensor([[-1], [0], [1]]).cuda().view(1, 1, 3, 1), padding=(1,0),stride=1)

        gradient = torch.sqrt(gradient_x**2 + gradient_y**2)

        # Optionally: Apply ReLU or other activation
        gradient = F.relu(gradient)

        # Optionally: Normalize weights to [0, 1]
        max_val = torch.max(gradient)
        gradient = gradient / max_val if max_val > 0 else gradient

        weights = self.sigmoid(self.conv(gradient))
        x = x * weights
        return x
...
class Disentangle_UNet(nn.Module):
    def __init__(self, n_channels=1, n_classes=1, bilinear=True):
        ...
        self.inc_low = DoubleConv(n_channels, 64)
        self.inc_high = DoubleConv(3, 64)
        self.down1 = Down(64, 128)
        self.down2 = Down(128, 256)
        self.down3 = Down(256, 512 // factor)
        self.down4 = Down(256, 1024 // factor)
        self.attention0 = Frequency_Attention()
        self.attention1 = Frequency_Attention()
        # self.attention2 = Frequency_Attention()
        self.attention3 = Frequency_Attention()
        # self.attention4 = Frequency_Attention()
        self.attention5 = Frequency_Attention()

    def forward(self, input, type='low'):
        feats = []
        #input = self.attention0(input)
        if(type == 'high'):
            x1 = self.inc_high(input)
        else:
            x1 = self.inc_low(input)
...
class PatchNCELoss(nn.Module):
    def __init__(self, N_patches=100, nce_t=1):
        super().__init__()
...
class DWT_transform(nn.Module):
    ...
    def forward(self, x):
        ...
        dwt_low_freq, dwt_high_freq = dwt_result[:,0:1*self.in_channels,:,:], dwt_result[:,1*self.in_channels:4*self.in_channels,:,:]
        dwt_low_freq = self.conv1x1_low(dwt_low_freq)
        dwt_high_freq = self.conv1x1_high(dwt_high_freq)
        return dwt_low_freq, dwt_high_freq

class PatchSampleF_CUT(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleF_CUT, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
